...
**CDN — Cloudflare since 2026-08-14 (was CloudFront from 2026-06-11)**: the apex is `A 16.112.156.196` **proxied by Cloudflare Free** (zone nameservers `becky`/`miles.ns.cloudflare.com`; DNS moved off Route 53, which Free requires). `www`/`beta`/`origin` stay gray (DNS-only, straight to Caddy); `image.themoviebrowser.com` stays on CloudFront (`E300L33VF15D5T`) and stays gray — Cloudflare's ToS restricts image-heavy serving on Free, and at ~1.65M req/mo it is inside CloudFront's free tier. **Rollback = point the apex back at `CNAME d1vtxoi7slst5n.cloudfront.net`, gray; the CloudFront distribution `E12R1ZNQNG3LK5` and the Route 53 zone are BOTH still alive for exactly that.** Why we moved: CloudFront bills per request and the scraper fleet took us to ~90M req/mo (~$66-90/mo); Cloudflare requests are unmetered. **All CDN work + footguns are in `.claude/rules/cdn.md` — read it before touching Cloudflare/CloudFront/Caddy/robots/next.config.**

**Main EC2 (production since GA 2026-06-10, now the Cloudflare origin)**: `m8g.large` (2 vCPU / 8GB ARM Graviton4, non-burstable; migrated 2026-06-19 from `t4g.large` to escape CPU-credit throttling — same in-place stop/start via TF, `ignore_changes=[ami,user_data]`+`prevent_destroy` keep it safe) with a 120GB gp3 root volume (grown from 80GB same day) in `ap-south-2` (Hyderabad). EIP `16.112.156.196`. Serves `themoviebrowser.com` + `www` + `beta.themoviebrowser.com` via Caddy (apex block also serves `origin.themoviebrowser.com`). Managed by Terraform (`terraform/`, state key `beta/terraform.tfstate`, project name `movie-browser-beta`).
...
**AI Agent (Cue)**: LangGraph agent with MemorySaver checkpointer. 12 tools (8 TMDB + `get_user_profile` + `get_community_buzz` (site-native reviews/ratings/discussion, anon-safe tier only) + `web_search` + `web_extract` via Tavily), Kimi K2.5 (knowledge cutoff: June 2025; model choice re-validated Jul 2026 — K2.6/K3 not on Bedrock). TMDB-first strategy — agent prefers free TMDB tools, web search only for current events/news/box office/reviews. Agent outputs `[SOURCE:url|title]` citation tags and `[WEB_IMAGE:url|description]` image tags for web results. Thread-based conversation persistence — frontend sends `threadId`, server restores full state (messages + tool calls + results). Tools receive `userId`, `pageContext`, and `userContext` (name, region, timezone) via `config.configurable`. `get_page_context` returns media metadata + user status (watched/watchlisted/rated). `get_user_profile` returns compact taste profile (top genres, recent watches, series progress, counts). `navigate_to` reaches every surface (detail pages, discussions, search results, watchlist/diary/stats/library, profiles). Guests get the full agent (guest-variant system prompt, no personalization tools); `/api/ai/chat` is Zod-validated + rate-limited server-side (`chat-rate-limit.ts`). Per-invocation logging isolated via `invocationId` Map. Recursion limit: 25. `TAVILY_API_KEY` required for web tools. See `.claude/rules/ai-agent.md`.
...
| `analytics-system.md` | `analytics/**`, `use-analytics.ts`, `api/analytics/**`, `admin/analytics/**` | Event tracking, cost tracking, ClickHouse queries, dashboard, the **Product tab** (confirmed-human floor; why `HUMAN_SQL` is NOT a product-metric scope), and why `session_id` is not a user |
