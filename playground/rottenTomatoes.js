...
async function scrapeRottenTomatoesInfo(rtId) {
  try {
    ...
    const posterImage =
      $('media-scorecard rt-img[slot="posterImage"]').attr("src") ||
      $('media-scorecard rt-img[slot="posterImage"]').attr("fallback-src") ||
      null;
    ...
    const criticsScore =
      $('media-scorecard rt-text[slot="criticsScore"]').first().text().trim() || null;
    const criticsReviewCount =
      $('media-scorecard rt-link[slot="criticsReviews"]')
        .text()
        .trim()
        .replace(/[^0-9]/g, "") || null;
    ...
    const audienceScore =
      $('media-scorecard rt-text[slot="audienceScore"]').first().text().trim() || null;
    const audienceRatingCount =
      $('media-scorecard rt-link[slot="audienceReviews"]').text().trim() || null;

    // Extract movie description
    const description =
      $('media-scorecard drawer-more rt-text[slot="content"]').text().trim() ||
    ...
    const movieInfo = {
      ...
      releaseYear: schemaData?.dateCreated
        ? schemaData.dateCreated.substring(0, 4)
        : $('[data-qa="movie-info-item-value"]').first().text().trim().match(/\d{4}/) || null,
      contentRating:
        schemaData?.contentRating ||
        $('[data-qa="movie-info-item-value"]')
          .filter((_, el) => $(el).text().includes("Rating"))
          .text()
          .trim() ||
        null,
      genres:
        schemaData?.genre ||
        $('[data-qa="movie-info-item-value"]')
          .filter((_, el) => $(el).prev().text().includes("Genre"))
          .text()
          .trim()
          .split(",")
          .map((g) => g.trim()) ||
