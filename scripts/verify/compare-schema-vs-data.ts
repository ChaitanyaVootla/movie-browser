...
const PRISMA_MOVIE_FIELDS = {
  // Core Movie table
  core: [
    "id", "title", "originalTitle", "overview", "adult", "posterPath", "backdropPath",
    "releaseDate", "runtime", "popularity", "status", "tagline", "budget", "revenue",
    "homepage", "originalLanguage", "originCountry", "collectionId",
  ],
...
const PRISMA_SERIES_FIELDS = {
  core: [
    "id", "name", "originalName", "overview", "adult", "posterPath", "backdropPath",
    "firstAirDate", "lastAirDate", "popularity", "status", "tagline", "type",
    "inProduction", "numberOfSeasons", "numberOfEpisodes", "episodeRunTime",
    "homepage", "originalLanguage", "originCountry",
    "lastEpisodeSeasonNum", "lastEpisodeNum", "lastEpisodeAirDate",
    "nextEpisodeSeasonNum", "nextEpisodeNum", "nextEpisodeAirDate",
  ],
...
function analyzeMovieData(data: DataAnalysis): void {
  ...
  const mongoExtIds = (mongo.external_data as Record<string, unknown>)?.externalIds as Record<string, unknown> || {};
  ...
  const mongoRatings = (mongo.external_data as Record<string, unknown>)?.ratings as Record<string, unknown> || {};
  ...
  console.log(`       ⚠️ sourceUrl: ${imdb.sourceUrl ? "present" : "null"} → Rating.sourceUrl (MIGRATION MISSING)`);
  
  // RT Critic
  const rtCritic = (mongoRatings.rottenTomatoes as Record<string, unknown>)?.critic as Record<string, unknown> || {};
  ...
  console.log(`       ⚠️ consensus: ${rtCritic.consensus ? "present" : "null"} → Rating.consensus (MIGRATION MISSING)`);
  console.log(`       ⚠️ sentiment: ${rtCritic.sentiment} → Rating.sentiment (MIGRATION MISSING)`);
  
  // RT Audience
  const rtAudience = (mongoRatings.rottenTomatoes as Record<string, unknown>)?.audience as Record<string, unknown> || {};
  ...
  console.log(`       ⚠️ sourceUrl: ${rtSourceUrl ? "present" : "null"} → Rating.sourceUrl (MIGRATION MISSING)`);
  ...
  if (videos.length > 0) {
    ...
    console.log(`     ⚠️ published_at: ${firstVideo.published_at ? "present" : "null"} → Video.publishedAt (MIGRATION MISSING)`);
  ...
  if (images.backdrops?.length) {
    ...
    console.log(`     ⚠️ vote_count: ${firstImage.vote_count} → Image.voteCount (MIGRATION MISSING)`);
    console.log(`     ⚠️ iso_639_1: ${firstImage.iso_639_1} → Image.language (MIGRATION MISSING)`);
  }
  
  // Release Dates / Certifications
  const releaseDates = (tmdb.release_dates as Record<string, unknown>)?.results as unknown[] || [];
  if (releaseDates.length > 0) {
    console.log("\n  📜 Certifications (release_dates):");
    console.log(`     ✅ Schema: MovieCertification with countryCode, certification, releaseDate, releaseType, note`);
    console.log(`     ⚠️ MIGRATION MISSING: seed-from-mongo.ts doesn't migrate releaseDates → MovieCertification`);
  }
  ...
  if (prodCountries.length > 0) {
    ...
    console.log(`     ⚠️ MIGRATION MISSING: seed-from-mongo.ts doesn't migrate production_countries`);
  }
  ...
  const watchProviders = mongo.watchProviders || (tmdb["watch/providers"] as Record<string, unknown>)?.results;
  if (watchProviders) {
    console.log("\n  📺 TMDB Watch Providers:");
...
function analyzeSeriesData(data: DataAnalysis): void {
  ...
  const contentRatings = (tmdb.content_ratings as Record<string, unknown>)?.results as unknown[] || [];
  ...
  const seriesExtIds = [
    "imdb_id", "tvdb_id", "tvrage_id", "freebase_mid", "freebase_id",
