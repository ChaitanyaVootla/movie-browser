#!/usr/bin/env npx tsx
/**
 * Series Content Enrichment Script
 *
 * Fetches and aggregates raw data from multiple sources for TV series.
 * Generates optimized, AI-ready markdown files.
 *
 * Usage:
 *   yarn enrich:series <tmdb_id>           # Single series
 *   yarn enrich:series 1399,1396,60735     # Multiple series (comma-separated)
 *   yarn enrich:series 1399 --debug        # Verbose output
 *
 * Output per series: ./data/enriched/<tmdb_id>/
...
function getMongoURI(): string {
  ...
  if (!mongoIp || !mongoPass) {
    console.warn("  MongoDB env vars not set - will use TMDB only");
    return "";
  }
...
interface TMDBSeriesData {
  ...
  type: string; // "Scripted", "Documentary", "Reality", etc.
  ...
  created_by: Array<{ id: number; name: string; gender: number; profile_path: string | null }>;
  seasons: Array<{
    id: number;
    name: string;
  ...
  recommendations: { results: Array<{ id: number; name: string; vote_average: number }> };
  similar: { results: Array<{ id: number; name: string; vote_average: number }> };
  videos: {
...
interface EnrichedSeriesContent {
  tmdbId: number;
  name: string;
  year: string;
  fetchedAt: string;
  tmdb: TMDBSeriesData | null;
  mongodb: MongoDBSeriesData | null;
  ...
  for (let i = 0; i < retries; i++) {
    try {
      ...
      if (response.status === 429) {
        ...
        console.log(`  Waiting ${retryAfter}s (rate limited)...`);
...
async function fetchFromMongoDB(tmdbId: number): Promise<MongoDBSeriesData | null> {
  if (mongoose.connection.readyState !== 1) {
    return null;
  }

  console.log(`  Fetching from MongoDB (ID: ${tmdbId})...`);

  try {
    const db = mongoose.connection.db;
    if (!db) return null;

    // Try 'series' collection first, then 'tv' as fallback
    let series = await db.collection("series").findOne({ id: tmdbId });
    if (!series) {
      series = await db.collection("tv").findOne({ id: tmdbId });
    }
    ...
    // Type-safe extraction of series data
    const seriesData = series as unknown as MongoDBSeriesData;

    const hasRatings = !!(
      seriesData.external_data?.ratings?.imdb?.rating ||
      seriesData.external_data?.ratings?.rottenTomatoes?.critic?.score ||
      seriesData.googleData?.ratings?.length
    );

    console.log(`    Found in MongoDB`);
    console.log(`    Name: ${seriesData.name}`);
    console.log(`    Has scraped ratings: ${hasRatings ? "Yes" : "No"}`);
...
// =============================================================================
// TMDB Fetcher (Series-specific)
// =============================================================================

async function fetchTMDB(tmdbId: number): Promise<TMDBSeriesData> {
  console.log(`  Fetching TMDB series ${tmdbId}...`);
  const url = `https://api.themoviedb.org/3/tv/${tmdbId}?api_key=${TMDB_API_KEY}&append_to_response=credits,keywords,external_ids,videos,recommendations,similar,content_ratings`;
  const response = await fetchWithRetry(url);
  const data = await response.json();
  log("TMDB response", data);
  console.log(`    Name: ${data.name} (${data.first_air_date?.split("-")[0] || "Unknown"})`);
  console.log(`    Status: ${data.status}`);
  console.log(`    Seasons: ${data.number_of_seasons}, Episodes: ${data.number_of_episodes}`);
  console.log(`    Genres: ${data.genres?.map((g: { name: string }) => g.name).join(", ")}`);
  console.log(`    Networks: ${data.networks?.map((n: { name: string }) => n.name).join(", ")}`);
  console.log(`    Keywords: ${data.keywords?.results?.length || 0}`);
  console.log(
    `    Cast: ${data.credits?.cast?.length || 0}, Crew: ${data.credits?.crew?.length || 0}`
  );
  return data as TMDBSeriesData;
}
...
async function fetchWikidata(tmdbId: number): Promise<WikidataData | null> {
  console.log(`  Searching Wikidata for TMDB TV ID ${tmdbId}...`);

  // Note: P4983 is TMDB TV series ID (different from P4947 which is movie ID)
  const query = `
    SELECT ?item ?itemLabel ?itemDescription
           (GROUP_CONCAT(DISTINCT ?alias; SEPARATOR="|") AS ?aliases)
           ?imdbId ?rottentomatoesId ?metacriticId ?tvdbId
           ?fandomArticle ?fandomWiki ?tvTropesId ?tvTropesArticle
           ?netflixId ?amazonId ?appleTvId ?disneyPlusId ?huluId
           ?googleKgId ?traktId
    WHERE {
      ...
      OPTIONAL { ?item wdt:P4835 ?tvdbId. }
      ...
      OPTIONAL { ?item wdt:P12156 ?huluId. }
    ...
    GROUP BY ?item ?itemLabel ?itemDescription ?imdbId ?rottentomatoesId
             ?metacriticId ?tvdbId ?fandomArticle ?fandomWiki ?tvTropesId ?tvTropesArticle
             ?netflixId ?amazonId ?appleTvId ?disneyPlusId ?huluId
             ?googleKgId ?traktId
    LIMIT 1
  `;
  ...
  try {
    ...
    if (!bindings) {
      console.log("    No Wikidata entity found for this TMDB TV ID");
      return null;
    }
    ...
    if (!entity) {
      console.log("    Could not fetch entity data");
      return null;
    }
...
async function scrapeFandom(
  ...
  for (const wiki of possibleWikis) {
    const url = `https://${wiki}.fandom.com/wiki/${encodeURIComponent(seriesName.replace(/ /g, "_"))}`;
  ...
  console.log("    No Fandom wiki found");
  return null;
}
...
function generateTMDBMarkdown(tmdb: TMDBSeriesData): string {
  const creators = tmdb.created_by?.map((c) => c.name).join(", ");
  ...
  const networks = tmdb.networks?.map((n) => n.name) || [];

  // Get average episode runtime
  const avgRuntime =
    tmdb.episode_run_time?.length > 0
      ? Math.round(tmdb.episode_run_time.reduce((a, b) => a + b, 0) / tmdb.episode_run_time.length)
      : null;

  let md = `# ${tmdb.name} (${tmdb.first_air_date?.split("-")[0]})\n\n`;
  ...
  md += `## Details\n`;
  md += `- **Status:** ${tmdb.status}\n`;
  md += `- **Seasons:** ${tmdb.number_of_seasons}\n`;
  md += `- **Episodes:** ${tmdb.number_of_episodes}\n`;
  if (avgRuntime) md += `- **Episode Runtime:** ~${avgRuntime} minutes\n`;
  md += `- **Genres:** ${tmdb.genres?.map((g) => g.name).join(", ")}\n`;
  md += `- **Rating:** ${tmdb.vote_average}/10 (${tmdb.vote_count.toLocaleString()} votes)\n`;
  if (networks.length) md += `- **Networks:** ${networks.join(", ")}\n`;
  if (creators) md += `- **Created By:** ${creators}\n`;
  md += `- **Original Language:** ${tmdb.original_language.toUpperCase()}\n`;
  if (tmdb.first_air_date) md += `- **First Air Date:** ${tmdb.first_air_date}\n`;
  if (tmdb.last_air_date && tmdb.status !== "Returning Series") {
    md += `- **Last Air Date:** ${tmdb.last_air_date}\n`;
  }
  md += "\n";
  ...
  if (tmdb.seasons?.length) {
    ...
    tmdb.seasons
      ...
      .forEach((s) => {
        const airYear = s.air_date?.split("-")[0] || "TBA";
        md += `- **Season ${s.season_number}** (${airYear}): ${s.episode_count} episodes\n`;
      });
    md += "\n";
  }
...
function generateWikipediaMarkdown(wiki: WikipediaData): string {
  ...
  // Prioritize plot/premise-related sections (TV series often use "Premise" or "Synopsis")
  const prioritySections = ["Plot", "Synopsis", "Premise", "Story", "Overview"];
  ...
  const tvSections = ["Episodes", "Seasons", "Cast and characters", "Cast"];
  ...
  const interestingSections = ["Themes", "Production", "Reception", "Development", "Broadcast"];
...
function generateCombinedAIInput(content: EnrichedSeriesContent): string {
  let md = `# ${content.name} (${content.year}) - TV Series\n\n`;
  ...
  // Status (TMDB only)
  if (tmdb?.status) md += `- **Status:** ${tmdb.status}\n`;

  // Season/Episode count: prefer MongoDB, fallback to TMDB
  const seasons = mongo?.number_of_seasons || tmdb?.number_of_seasons;
  const episodes = mongo?.number_of_episodes || tmdb?.number_of_episodes;
  if (seasons) md += `- **Seasons:** ${seasons}\n`;
  if (episodes) md += `- **Episodes:** ${episodes}\n`;

  // Episode runtime (TMDB only)
  if (tmdb?.episode_run_time?.length) {
    const avgRuntime = Math.round(
      tmdb.episode_run_time.reduce((a, b) => a + b, 0) / tmdb.episode_run_time.length
    );
    md += `- **Episode Runtime:** ~${avgRuntime} minutes\n`;
  }

  // Networks: prefer MongoDB, fallback to TMDB
  const networks = mongo?.networks || tmdb?.networks;
  if (networks?.length) md += `- **Networks:** ${networks.map((n) => n.name).join(", ")}\n`;

  // Created by (TMDB only)
  if (tmdb?.created_by?.length) {
    md += `- **Created By:** ${tmdb.created_by.map((c) => c.name).join(", ")}\n`;
  }
  ...
  // Season breakdown (TMDB only)
  if (tmdb?.seasons?.length) {
    const regularSeasons = tmdb.seasons.filter((s) => s.season_number > 0);
    if (regularSeasons.length > 0) {
      md += `## Seasons\n`;
      regularSeasons.forEach((s) => {
        const airYear = s.air_date?.split("-")[0] || "TBA";
        md += `- **Season ${s.season_number}** (${airYear}): ${s.episode_count} episodes`;
        if (s.overview) md += ` - ${truncate(s.overview, 100)}`;
        md += "\n";
      });
      md += "\n";
    }
  ...
  if (content.wikipedia) {
    ...
    // TV series often use "Premise" instead of "Plot"
    const plotSection = w.sections["Plot"] || w.sections["Premise"] || w.sections["Synopsis"];
    if (plotSection) {
      md += `## Premise\n${plotSection}\n\n`;
    }
  ...
  if (content.imdb) {
    ...
    const hasGoodPlot =
      (content.wikipedia?.sections?.["Plot"]?.length ?? 0) > 500 ||
      (content.wikipedia?.sections?.["Premise"]?.length ?? 0) > 500;
    ...
    // Quotes capture the show's voice
    if (i.quotes.length) {
      md += `## Notable Quotes\n`;
...
function saveContent(content: EnrichedSeriesContent) {
  ...
  const metadata: Metadata = {
    ...
    stats: {
      ...
      hasPlot: !!(
        content.wikipedia?.sections?.["Plot"] ||
        content.wikipedia?.sections?.["Premise"] ||
        content.imdb?.synopsis
      ),
      hasTrivia: !!content.imdb?.trivia?.length,
      hasCategories: !!content.wikipedia?.categories?.length,
      numberOfSeasons: content.tmdb?.number_of_seasons || 0,
      numberOfEpisodes: content.tmdb?.number_of_episodes || 0,
    },
  };
...
async function enrichSeries(tmdbId: number): Promise<EnrichedSeriesContent> {
  console.log(`\n${"=".repeat(70)}`);
  console.log(`SERIES ENRICHMENT: TMDB ID ${tmdbId}`);
  ...
  await sleep(500);
  fandom = await scrapeFandom(wikidata?.externalIds?.fandom_wiki, tmdb.name);
  ...
  // Use MongoDB name/year if available (with TMDB fallback)
  const name = mongodb?.name || tmdb.name;
  const year = (mongodb?.first_air_date || tmdb.first_air_date)?.split("-")[0] || "Unknown";
...
async function main() {
  ...
  if (args.length === 0) {
    console.log(`
Usage: yarn enrich:series <tmdb_id> [--debug]
       yarn enrich:series <id1>,<id2>,<id3>  # Batch mode

Examples:
  yarn enrich:series 1399           # Game of Thrones
  yarn enrich:series 1399 --debug   # Verbose output
  yarn enrich:series 1399,1396,60735  # Batch: GoT, Breaking Bad, The Flash

Data Sources:
  - MongoDB (primary): Scraped ratings (IMDb, RT, Google), existing series data
  - TMDB (fallback/supplement): Keywords, videos, fresh metadata
  - Wikidata: External IDs, sitelinks
  - Wikipedia: Plot/Premise, themes, reception, categories
...
`);
  ...
  for (const tmdbId of tmdbIds) {
    try {
      const content = await enrichSeries(tmdbId);
      saveContent(content);
      ...
      results.push({
        ...
        hasRatings,
        seasons: content.tmdb?.number_of_seasons || 0,
        episodes: content.tmdb?.number_of_episodes || 0,
        success: true,
      });
      ...
      console.log("=".repeat(70));
      console.log(`   Name: ${content.name} (${content.year})`);
      console.log(`   Status: ${content.tmdb?.status}`);
      console.log(
        `   Seasons: ${content.tmdb?.number_of_seasons}, Episodes: ${content.tmdb?.number_of_episodes}`
      );
      console.log(`   AI Input: ~${estimateTokens(aiInput).toLocaleString()} tokens`);
      console.log(`   MongoDB data: ${content.mongodb ? "Yes" : "No"}`);
      console.log(`   Multi-source ratings: ${hasRatings ? "Yes" : "No"}`);
      console.log(
        `   Has plot/premise: ${
          content.wikipedia?.sections?.["Plot"] ||
          content.wikipedia?.sections?.["Premise"] ||
      ...
      console.log(`   Has trivia: ${content.imdb?.trivia?.length ? "Yes" : "No"}`);
      console.log(`   Has categories: ${content.wikipedia?.categories?.length ? "Yes" : "No"}`);

      if (tmdbIds.length > 1) {
        console.log(`\n  Waiting before next series...`);
        await sleep(2000);
      }
    } catch (error) {
  ...
  if (tmdbIds.length > 1) {
    ...
    const totalSeasons = successful.reduce((sum, r) => sum + r.seasons, 0);
    const totalEpisodes = successful.reduce((sum, r) => sum + r.episodes, 0);
    console.log(`   Processed: ${successful.length}/${tmdbIds.length} series`);
    console.log(`   With multi-source ratings: ${withRatings.length}/${successful.length}`);
    console.log(`   Total seasons: ${totalSeasons}, Total episodes: ${totalEpisodes}`);
    ...
    results.forEach((r) => {
      ...
      const ratings = r.hasRatings ? "(ratings)" : "";
      console.log(
        `   ${status} ${r.id}: ${r.name} - ${r.seasons}S/${r.episodes}E (~${r.tokens.toLocaleString()} tokens) ${ratings}`
      );
    });
  }
