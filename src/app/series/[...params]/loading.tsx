...
/**
 * Series page loading skeleton with REAL CDN images
 *
 * Key insight: CDN URLs are deterministic - only need the series ID
 * Since images are often cached from home/browse pages, they load instantly
 * Only dynamic content (ratings, genres, episodes) shows skeleton
 */
export default function SeriesLoading() {
  const params = useParams();
  // Extract series ID from route params (first segment)
  const seriesId = params?.params?.[0] as string | undefined;
  const id = seriesId ? parseInt(seriesId, 10) : null;
  ...
  return <LoadingWithImages seriesId={id} />;
}

// Loading state with real CDN images
function LoadingWithImages({ seriesId }: { seriesId: number }) {
