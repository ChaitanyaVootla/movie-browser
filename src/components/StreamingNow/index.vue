<template>
    <div>
            ...
            <el-row>
                <el-col :span="4" :offset="20" class="mt-1">
                    ...
                    <el-select v-model="selectedNetwork" filterable placeholder="Select">
                        <el-option v-for="item in allNetworks" :key="item.id" :label="item.name" :value="item.id">
                        </el-option>
                    </el-select>
                </el-col>
            </el-row>
        ...
        <mb-slider
            ...
            v-if="currentStreaming.length > 0"
        >
            <div class="slot-text">
            ...
            <div v-for="network in userNetworksData" :key="network.id">
                ...
                >
                    <el-row>
                        <el-col :span="12">
                            <img
                                :src="`${configuration.images.secure_base_url}w500${network.image}`"
                                class="slot-logo"
                                v-if="network.image"
                            />
        ...
        <!-- Default networks -->
        <mb-slider
            :items="topNetflixSeries"
            :configuration="configuration"
            :id="'netflixShows'"
            :showMovieInfoModal="showMovieInfo"
            :showFullMovieInfo="showFullMovieInfo"
            v-if="topNetflixSeries.length > 0"
        >
        ...
        <mb-slider
            :items="topHBOSeries"
            :configuration="configuration"
            :heading="'Top Shows on HBO'"
            :id="'amazonShows'"
            :showMovieInfoModal="showMovieInfo"
            :showFullMovieInfo="showFullMovieInfo"
            v-if="topHBOSeries.length > 0"
        >
            <img :src="`${configuration.images.secure_base_url}w500${NETWORKS.AMAZON.image}`" class="slot-logo" />
        </mb-slider>
        <mb-slider
            :items="topAmazonSeries"
            :configuration="configuration"
            :heading="'Top Shows on Amazon'"
            :id="'amazonShows'"
            :showMovieInfoModal="showMovieInfo"
            :showFullMovieInfo="showFullMovieInfo"
            v-if="topAmazonSeries.length > 0"
        >
            <img
                :src="`${configuration.images.secure_base_url}w500${NETWORKS.HBO.image}`"
                class="slot-logo invert-logo"
            />
        </mb-slider>
    </div>
</template>
