<template>
    <div>
        ...
        <div class="p-3">
            ...
            <span v-if="item.overview.length > 200" class="expand-ellipsis ml-3" @click="showFullOverview = !showFullOverview">...</span>
        </div>
    </div>
</template>
    ...
    export default {
        ...
        data() {
            return {
                ...
                bgImageObj: {
                    src: this.configuration.images.secure_base_url + 'w300' + (this.item.backdrop_path || this.item.backdropPath),
        ...
        computed: {
            googleLink() {
                return `https://google.com/search?q=${this.item.name || this.item.title} ${this.item.first_air_date?'tv series':this.getYear(this.item.release_date) + ' movie'}`;
            },
        },
    }
</script>
