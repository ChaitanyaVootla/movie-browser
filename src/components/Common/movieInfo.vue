            ...
            <div class="info-main">
                <h3 div="info-heading" v-if="isLoaded">
                    ...
                    <span class="text-muted info-tagline" v-if="movie.movieDetails.numberOfSeasons">
                        {{ movie.movieDetails.numberOfSeasons }} Season{{
                            movie.movieDetails.numberOfSeasons > 1 ? 's' : ''
                        }}
                    </span>
                </h3>
                <font-awesome-icon
                ...
                <!-- External links -->
                <a
                    :href="`https://google.com/search?q=${movie.movieDetails.name} ${getYear(
                ...
                >&nbsp;
                <a
                    v-if="movie.movieDetails && movie.movieDetails.homepage"
                ...
                <div v-if="isLoaded" style="margin-top: 2em">
                    <span
                        class="rating-info"
                        :style="`border-color: ${getRatingColor(movie.vote_average)}; color: ${getRatingColor(
                            movie.vote_average,
                        )}`"
                    >
                        {{ movie.vote_average }}
                ...
                <!-- Trailer/Video -->
                <div
                    v-if="isLoaded && getYoutubeVideos(movie.movieDetails.videos).length"
                ...
                >
                    <iframe
                        ...
                        :src="`https://www.youtube.com/embed/${
                            selectedVideo.key || getYoutubeVideos(movie.movieDetails.videos)[0].key
                        }`"
                        frameborder="0"
                        iv_load_policy="3"
                        fs="1"
                    ...
                    <div class="dropdown">
                        ...
                        <div class="dropdown-menu dropdown-menu-middle" aria-labelledby="dropdownMenuButton">
                            <a
                                class="dropdown-item"
                                v-for="video in movie.movieDetails.videos"
                                :key="video.key"
                ...
                <div v-if="isLoaded && getDirectors(movie.movieDetails.credits.crew).length > 0 && movie.movieDetails">
                    <h5
                        style="margin-top: 2em; margin-bottom: -0.6em"
                        v-if="getDirectors(movie.movieDetails.credits.crew).length > 0"
                    >
                                ...
                                <img
                                    v-lazy="
                                        director.profile_path
                                            ? creditImageBasePath + director.profile_path
                                            : 'https://via.placeholder.com/150/000/000?'
                                    "
                                    class="credit-img"
                                    :title="`${director.name}`"
                                />
                ...
                <!-- Cast -->
                <h5
                    style="margin-top: 3em; margin-bottom: -0.6em"
                    v-if="movie.movieDetails && movie.movieDetails.credits.cast.length > 0"
                >
                    Cast
                </h5>
                <div class="credits-container" v-if="isLoaded">
                    <div v-for="actor in movie.movieDetails.credits.cast.slice(0, 10)" :key="actor.name">
                        <div class="credit-item" v-if="actor.profile_path" v-on:click="googleCredit(actor)">
                            <img
                                v-lazy="creditImageBasePath + actor.profile_path"
                                class="credit-img"
    ...
    methods: {
        ...
        cycleBgs() {
            ...
            this.bgInterval = setInterval(() => {
                ...
                $(`#${this.movie.id} .backdrop`).css(
                    'background-image',
                    `${this.bgStyle},
                        url(${
                            this.originalimageBasePath +
                            this.movie.movieDetails.images.backdrops[this.bgIndex].file_path
                        })`,
                );
            }, 10000);
        },
