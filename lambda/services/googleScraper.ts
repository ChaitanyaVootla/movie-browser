...
export class GoogleScraperService {
  ...
  private async setupPage(page: Page) {
    ...
    if (isChrome) {
      ...
      headers["sec-ch-ua-platform-version"] =
        fingerprint.platform === "Win32" ? '"15.0.0"' : '"14.6.1"';
    }
  ...
  async scrape(searchString: string, region?: string): Promise<GoogleScraperResult> {
    ...
    try {
      ...
      await this.page.evaluate(async () => {
        ...
        let currentScroll = 0;
        let scrollCount = 0;
        while (currentScroll < maxScroll && scrollCount < 5) {
          // Limit scrolls
      ...
      const isDetected = detectionIndicators.some(
        (indicator) =>
      ...
      if (isDetected) {
        this.retryCount++;
        logger.warn(
          `Bot detected (attempt ${this.retryCount}/${MAX_RETRIES}). Page title: ${pageTitle}`
        );
        ...
        throw new Error(
          `Bot detection limit reached after ${MAX_RETRIES} retries. Last page title: ${pageTitle}`
        );
      }
  ...
  static async scrapeGoogleData(
    searchString: string,
    region?: string
  ): Promise<GoogleScraperResult | null> {
