    ...
    try {
      ...
      $('script[type="application/ld+json"]').each((_, el) => {
        try {
          ...
          if (
            (jsonData["@type"] === "Movie" || jsonData["@type"] === "TVSeries") &&
          ...
          if (jsonData.audience?.aggregateRating) {
            ...
            jsonAudienceReviewCount = parseInt(
              String(jsonData.audience.aggregateRating.ratingCount),
              10
            );
          }
      ...
      if (scoreCard.length > 0) {
        ...
        const criticCountText = scoreCard
          .find('rt-link[slot="criticsReviews"]')
          .first()
          .text()
          .trim();
        fallbackCriticReviewCount = criticCountText
          ? parseInt(criticCountText.replace(/[^0-9]/g, ""), 10)
          : null;

        // Extract audience score
        const audienceScoreText = scoreCard
          .find('rt-text[slot="audienceScore"]')
          .first()
          .text()
          .trim();
        fallbackAudienceScore = audienceScoreText
          ? parseInt(audienceScoreText.replace("%", ""), 10)
          : null;

        // Extract audience review count
        const audienceCountText = scoreCard
          .find('rt-link[slot="audienceReviews"]')
          .first()
          .text()
          .trim();
        const audienceMatch = audienceCountText.match(/^[0-9,]+/);
        fallbackAudienceReviewCount = audienceMatch
          ? parseInt(audienceMatch[0].replace(/,/g, ""), 10)
          : null;
      ...
      const validatedCriticScore =
        finalCriticScore !== null && !isNaN(finalCriticScore) ? finalCriticScore : null;
      const validatedCriticCount =
        finalCriticCount !== null && !isNaN(finalCriticCount) ? finalCriticCount : null;
      ...
      return {
        critic:
          criticData.score !== null ||
          criticData.ratingCount !== null ||
          criticData.consensus !== null
            ? criticData
            : null,
        audience:
          audienceData.score !== null ||
          audienceData.ratingCount !== null ||
          audienceData.consensus !== null
            ? audienceData
            : null,
        sourceUrl: url,
        error: null,
