      ...
      <div class="video w-2/3 video" :key="currentVideo.key">
        ...
        <div class="flex gap-5 mt-4 justify-between px-2">
          <div class="text-neutral-200 overflow-ellipsis whitespace-nowrap overflow-hidden pr-4">
            ...
            <div class="text-neutral-300">
              <span class="font-semibold mr-2"
                >{{ formatNumber(videoStats?.viewCount || 0) }} views</span
              >
          ...
          <div>
            ...
            <div class="pt-1">
              <v-progress-linear
                :model-value="getVideoLikePercentage()"
                class="mt-2"
                :height="2"
                bg-opacity="0.3"
                aria-label="Likes"
        ...
        <div
          class="mt-5 bg-neutral-800 rounded-2xl px-4 py-2 cursor-pointer"
        ...
        >
          ...
          <div class="flex gap-3 items-center">
            ...
            <div class="flex flex-col">
              <div
                class="text-neutral-300 line-clamp-1"
                v-html="comments[0].snippet.topLevelComment.snippet.textDisplay"
              ></div>
            </div>
          </div>
        </div>
      ...
      <div class="px-10 flex w-1/3 flex-col overflow-auto">
        ...
        <div class="flex flex-col overflow-auto video-scroller">
          <div v-for="video in filteredVideos">
            <div
              class="flex py-2 pl-2 cursor-pointer hover:bg-neutral-900"
              :class="{ currentVideo: video.key === currentVideo.key }"
              :key="video.key"
                ...
                <div class="text-neutral-200 overflow-ellipsis overflow-hidden pr-4 text-tiny">
                  ...
                  <div class="mt-3 text-neutral-400">
                    ...
                    <div class="mt-1 text-sm">
                      <span class="font-semibold mr-2"
                        >{{ formatNumber(video.statistics?.viewCount || 0) }} views</span
                      >
      ...
      <div class="h-[40rem] overflow-y-auto">
        <div v-for="comment in comments" :key="comment.id" class="mb-5">
          <div class="flex gap-3">
            ...
            <div class="flex flex-col">
              <div class="text-neutral-200 font-semibold">
                {{ comment.snippet.topLevelComment.snippet.authorDisplayName }}
              </div>
              <div
                class="text-neutral-300 line-clamp"
          ...
          <div class="text-xs ml-13 font-thin flex items-center mt-1">
            <v-icon icon="mdi-thumb-up-outline" class="mr-2"></v-icon>
            <b class="mt-1">{{
              formatNumber(comment.snippet.topLevelComment.snippet.likeCount || 0)
            }}</b>
          </div>
        </div>
      </div>
...
const updateVideoStatistics = async () => {
  videoStats.value = await $fetch(
    `https://returnyoutubedislikeapi.com/votes?videoId=${currentVideo.value.key}`
  );
