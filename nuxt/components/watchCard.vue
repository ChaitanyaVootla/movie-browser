    ...
    <div class="w-44 md:w-80 group cursor-pointer pt-2 flex flex-col">
      <NuxtLink :to="item.watchLink" target="blank" noreferrer noopener>
        <SeoImg
          ...
          cover
          class="image rounded-lg hover:rounded-md hover:shadow-md hover:shadow-neutral-800 hover:transition-all duration-300 hover:mb-1 md:hover:-mt-1 w-full h-auto"
          :aspect-ratio="1.77"
          @error="imageError"
        >
      ...
      >
        <div
          class="overflow-ellipsis whitespace-nowrap overflow-hidden mt-1 text-neutral-200 text-2xs md:text-sm flex items-center gap-2"
        >
          <div>
            <SeoImg
              :src="watchOptionImageMapper[item.watchProviderName]?.image"
              class="max-md:w-6 max-md:h-6 md:w-7 md:h-7"
