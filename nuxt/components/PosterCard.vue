<template>
  ...
  <IntersectionLoader
    class="max-md:aspect-[2/3]"
    height="28rem"
    width="15rem"
    mobileHeight="auto"
    :mobileWidth="fluid ? '100%' : '9rem'"
    :eager="true"
  >
    <div ref="cardContainer" class="relative" @mouseenter="onMouseEnter" @mouseleave="onMouseLeave">
      <NuxtLink
        :to="`/${item.title ? 'movie' : 'series'}/${item.id}/${getUrlSlug(item.title || item.name)}`"
        v-memo="[item.id, watched, inWatchList]"
      >
        <div class="card group cursor-pointer flex flex-col">
          <div
            class="relative md:hover:mb-1 md:hover:-mt-1 hover:transition-all duration-300"
            :class="{ 'scale-95': watched }"
          >
            <SeoImg
              ...
              :class="{
                'saturate-0 opacity-80 border-neutral-500 border-2 shadow-lg shadow-neutral-700':
                  watched,
              }"
            ...
            <div v-if="isMovie(props.item) && isMounted" class="absolute bottom-0 w-full p-0">
              <v-btn
                ...
                :class="{
                  '!hidden group-hover:!block': !inWatchList,
                  '!border-neutral-500': inWatchList,
                }"
                icon="mdi-plus"
                rounded
                size="x-small"
            ...
            <div
              v-if="isAiRoute"
              v-once
              class="overlay invisible group-hover:visible absolute bottom-0 flex justify-center w-full z-10 px-3 pt-4 items-center"
            >
              <div class="flex items-center justify-between w-full">
                ...
                <v-btn
                  ...
                  @click.prevent="addToParentFilter()"
                  size="small"
                />
              </div>
            </div>
          ...
          <h3
            class="title overflow-ellipsis whitespace-nowrap overflow-hidden mt-1 group-hover:mt-4 text-neutral-200 hidden md:block text-sm"
          >
            {{ item.character || item.job || item.title || item.name }}
          </h3>
          <div
            v-if="item.infoText"
            class="text-neutral-400 text-2xs md:text-sm capitalize hidden md:block"
          >
            {{ item.infoText }}
          </div>
        </div>
      ...
      <!-- Quick View Modal -->
      <QuickViewModal v-if="fullItem" v-model="showModal" :item="fullItem" />
    </div>
  </IntersectionLoader>
</template>
