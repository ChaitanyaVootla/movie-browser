<template>
  <Scroller
    ref="scrollerRef"
    :items="filteredScrollItems"
    :title="localScrollItem.name"
    :pending="pending"
  >
      ...
      <div class="flex items-center justify-between md:pr-14">
        <slot name="title">
          <div v-if="!props.scrollItem.isPromo">
            <NuxtImg
              v-if="localScrollItem.logo"
              :src="localScrollItem.logo"
              :alt="localScrollItem.name"
              class="object-cover -m-0"
              height="20"
              width="100"
              :class="localScrollItem.name"
            />
...
const changeSort = () => {
  localScrollItem.value.filterParams.sort_by =
    sortOrder.value === 0 ? "popularity.desc" : "release_date.desc";
  resetAndRefresh();
};
...
const { stop } = useIntersectionObserver(scrollerRef, debouncedIntersectionHandler, {
