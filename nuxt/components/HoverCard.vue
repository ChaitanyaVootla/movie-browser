      ...
      <div class="flex flex-col w-full h-full relative">
        ...
        <div class="flex-grow p-4 flex flex-col gap-3 bg-black select-none overflow-hidden">
          ...
          <div class="flex items-center gap-2" v-if="isExpanded">
            ...
            <div
              v-if="videoStats"
              class="flex items-center bg-neutral-800 rounded-full px-3 py-1 text-xs gap-2 flex-shrink-0"
            >
            ...
            <div class="flex gap-2">
              <v-btn
                ...
                variant="outlined"
                color="white"
                class="!border-neutral-600 bg-neutral-800/80"
                :title="inWatchList ? 'Remove from Watchlist' : 'Add to Watchlist'"
                @click.stop="toggleWatchList"
              >
                <v-icon :color="inWatchList ? 'primary' : 'white'">{{
                  inWatchList ? "mdi-check" : "mdi-plus"
                }}</v-icon>
              </v-btn>
              <v-btn
                v-if="isMovieItem"
              ...
              >
                <v-icon :color="watched ? 'primary' : 'white'">{{
                  watched ? "mdi-eye" : "mdi-eye-outline"
                }}</v-icon>
              </v-btn>
              <v-btn
                icon
          ...
          <div
            v-if="isExpanded && item.watch_options"
            class="scale-75 origin-left mt-2"
            @click.stop
          >
            <WatchOptions :item="item" :compact="true" />
          </div>
          ...
          <div
            v-if="isExpanded"
            class="flex flex-nowrap items-center gap-x-3 text-xs text-neutral-400 mt-auto mb-2 overflow-hidden"
          >
            <span v-if="year" class="text-white shrink-0">{{ year }}</span>
            <span v-if="item.runtime" class="text-white shrink-0">{{
            ...
            <div
              class="flex flex-nowrap items-center gap-x-3 overflow-hidden"
              v-if="item.genres?.length"
            >
              <span
                v-for="genre in visibleGenres"
                :key="genre.id"
                class="text-neutral-500 whitespace-nowrap shrink-0"
              >
                {{ genre.name }}
              </span>
            </div>
...
watch([() => props.item, isExpanded], async ([newItem, expanded]) => {
  ...
  if (trailer?.key) {
    try {
      videoStats.value = await $fetch(
        `https://returnyoutubedislikeapi.com/votes?videoId=${trailer.key}`
      );
    } catch (e) {
