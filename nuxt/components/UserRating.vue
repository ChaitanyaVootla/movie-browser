  ...
  <div>
    <div class="flex items-center bg-neutral-800 rounded-full px-4 py-[6px] cursor-pointer">
      <div @click="likeClicked" class="flex items-center gap-2">
        <span
          class="material-symbols-outlined !text-[22px] md:!text-3xl"
          :style="`font-variation-settings: 'FILL' ${liked ? 1 : 0};`"
          >thumb_up</span
        >
        <!-- {{  itemLikes }} -->
      </div>
      ...
      <div @click="dislikeClicked" class="flex items-center gap-2">
        <span
          class="material-symbols-outlined !text-[22px] md:!text-3xl"
          :style="`font-variation-settings: 'FILL' ${disliked ? 1 : 0};`"
          >thumb_down</span
        >
        <!-- {{  itemDislikes }} -->
      </div>
...
const getUserRating = async () => {
  ...
  const userRating: any = await $fetch(
    `/api/user/ratings/userItemRating?itemType=${props.itemType}&itemId=${props.itemId}`,
    { headers }
  );
