<template>
  ...
  <IntersectionLoader
    height="30rem"
    width="36rem"
    mobileHeight="calc((80vw * 0.5625) + 12rem)"
    mobileWidth="80vw"
    :eager="true"
  >
    <NuxtLink
    ...
    >
      <div class="promo-img">
        <SeoImg
          :sources="
            [
              imagePath,
              `https://image.tmdb.org/t/p/${configuration.images.backdrop_sizes.w1280}${props.item.backdrop_path}`,
            ].filter(Boolean)
          "
      ...
      <div class="h-32 p-4">
        <div
          v-if="logo"
          class="title-logo logo-shadow w-full h-full flex justify-center items-center"
        >
          <SeoImg
            :sources="
              [
                logoPath,
                `https://image.tmdb.org/t/p/${configuration.images.backdrop_sizes.w1280}${logo}`,
              ].filter(Boolean)
            "
            :alt="item.title || item.name"
