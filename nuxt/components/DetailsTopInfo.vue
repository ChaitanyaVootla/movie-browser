  ...
  <div class="details-container bg-black">
    <div class="hidden md:flex w-full h-full">
      <div class="w-full md:w-[45%] pl-16 flex flex-col justify-between h-full pt-10 left-info">
        ...
        <div class="w-full h-1/2 flex mb-3 flex-col gap-5 justify-end">
          ...
          <div v-if="false" class="flex gap-5">
            <div
              v-for="cast in item.credits?.cast?.slice(0, 5) || []"
              class="flex flex-col justify-start w-24 items-center"
            >
              <NuxtImg
              ...
              />
              <div
                class="mt-1 text-neutral-400 text-xs overflow-ellipsis whitespace-nowrap overflow-hidden"
              >
                {{ cast.name }}
              </div>
            </div>
      ...
      <div class="hidden md:block w-[55%] h-full">
        <div
          v-if="!showTrailer"
          class="group image-width h-full w-full relative image-container cursor-pointer md:after:mr-14"
        ...
        >
          <div>
            <NuxtImg
              ...
              @error="onError"
              :alt="item.title || item.name"
              class="bg-main object-cover object-top image-width h-full w-full absolute md:pr-14"
            >
            </NuxtImg>
          </div>
          <v-btn
            v-if="item.youtubeVideos?.length"
            color="primary"
            class="rounded-pill !absolute bottom-5 right-20 z-10 group-hover:scale-110"
    ...
    <div class="md:hidden">
      <div class="">
        <div
          class="bg-mobile-container relative w-full h-full"
          v-if="!showMobileTrailer"
          @click="minimal ? '' : (showMobileTrailer = !showMobileTrailer)"
        >
          <div>
            <NuxtImg
              :src="imagePath"
              @error="onError"
              :alt="item.title || item.name"
              class="bg-mobile object-top image-width object-cover w-full"
            >
            </NuxtImg>
          </div>
      ...
      <div class="text-white font-bold text-2xl flex justify-center items-center">
        <NuxtImg
          ...
          @error="onLogoError"
          :alt="item.title || item.name"
          class="object-contain h-20 w-4/5 logo-shadow"
        />
        <div v-else>
          {{ item.title || item.name }}
        </div>
