<template>
  <div v-if="currentWatchOptions.length" class="flex">
    <div
      class="flex items-center justify-center flex-wrap max-md:gap-3 md:gap-4 bg-neutral-900 border-[1px] border-neutral-700 max-md:pl-2 max-md:pr-7 md:pl-4 md:pr-8 max-md:pt-4 max-md:pb-2 md:pt-6 md:pb-2 rounded-2xl relative min-w-32"
    >
      <div
        class="absolute w-full max-md:-top-2 -top-4 flex max-md:justify-center md:justify-start md:ml-5"
      >
        <div
          class="bg-neutral-700 pl-1 pr-2 py-0 rounded-full text-xs font-light flex items-center text-nowrap"
        >
          <span
            class="material-symbols-outlined !text-[22px] md:!text-4xl text -m-1"
          ...
          <div>
            Watch Now
            <div
              v-if="sourceCountry && sourceCountry !== selectedCountry.code"
              class="text-[10px] text-neutral-400 mt-1 leading-none"
            >
          ...
          <v-menu v-if="false" v-model="isOpen" :close-on-content-click="false">
            <template v-slot:activator="{ props }">
              <div
                v-bind="props"
                class="px-2 py-1 flex items-center rounded-full cursor-pointer gap-2"
              >
                <SeoImg
      ...
      <div v-for="watchOption in visibleWatchOptions">
        ...
        >
          <div class="w-18 flex flex-col items-center justify-between">
            <SeoImg
              :src="watchOption.image"
              :class="props.compact ? 'w-8 h-8' : 'max-md:w-6 max-md:h-6 md:w-6 md:h-6'"
              class="rounded-sm"
              :alt="watchOption.name"
            ></SeoImg>
            <div
              v-if="!props.compact && watchOption?.displayName"
              class="text-2xs md:text-xs text-neutral-200 text-center mt-1"
            >
              {{ watchOption.displayName }}
            </div>
            <div
              v-if="!props.compact && watchOption?.price?.length"
              class="text-2xs text-neutral-400 text-center"
            >
              {{ watchOption.price }}
            </div>
          </div>
      ...
      <div
        v-if="currentWatchOptions.length > 5"
    ...
    Object.entries({
      ...
      (providers || []).forEach((provider: any) => {
        if (!mappedProviders.find((item: any) => item.key === provider.provider_id)) {
          ...
          const existingProvider = mappedProviders.find(
            (item: any) => item.key === provider.provider_id
          );
...
if (props.item.watch_options) {
  ...
  scrapedWatchOptions = (props.googleData.allWatchOptions || [])
    .map((watchOption: any) => {
...
const countries = Object.keys(watchOptionsByCountry).map(
  (country) => getName(country) || "Unknown"
);
...
const watchLinkClicked = (watchOption: any) => {
  ...
  const englishBackdrop = props.item?.images?.backdrops?.find(
    ({ iso_639_1 }: any) => iso_639_1 === "en"
  )?.file_path;
