<template>
  <div
    class="navbar fixed w-full hidden md:flex justify-center text-xl items-center px-16 overflow-visible bg-black py-2 z-50 h-14"
  >
    <div class="left-actions items-center flex-1 flex gap-10 text-[1rem] font-medium">
      <NuxtLink to="/" aria-label="Go Home">
        <div
          class="flex items-start gap-1 tracking-widest text-xl text-white font-extrabold group mr-7"
        >
          <NuxtImg
            src="/popcorn-lite.png"
            class="w-7 h-7 shrink-0 group-hover:rotate-6 group-hover:scale-110 transition-all duration-200"
      ...
      <NuxtLink to="/topics">
        <div class="flex items-center gap-1 text-[14px]" aria-label="Go To Dynamic Lists">
          <span
            class="material-symbols-outlined !text-[22px] md:!text-xl text"
            style="font-variation-settings: &quot;FILL&quot; 1"
            >view_object_track</span
          >
          Topics
        </div>
      </NuxtLink>
      ...
      <NuxtLink to="/admin" v-if="isAdmin">
        <div
          class="flex items-center gap-1 whitespace-nowrap text-[14px]"
    ...
    <div class="right flex-1 flex justify-end">
      <div class="flex items-center gap-5">
        <v-chip
          class="rounded-xl px-5 py-4 cursor-pointer flex"
        ...
        <div>
          <div v-if="status === 'unauthenticated' || status === 'loading'">
            <NuxtImg
              @click="signIn('google')"
              src="/images/googleLogin/login.svg"
          ...
          <div class="flex items-center gap-6" v-else>
            <v-menu>
              ...
              <div class="mt-1 !rounded-lg !bg-neutral-800 min-w-60">
                ...
                <v-list>
                  <v-list-item>
                    <NuxtLink to="/watchList"> Watch List </NuxtLink>
                  </v-list-item>
                  <v-list-item>
                    <NuxtLink to="/ratings"> My Ratings </NuxtLink>
                  </v-list-item>
                  <v-list-item>
                    <NuxtLink to="/watched"> Watch History </NuxtLink>
                  </v-list-item>
                  <v-list-item @click="signOut" class="text-red"> Sign Out </v-list-item>
                </v-list>
              </div>
            </v-menu>
          </div>
  ...
  <div class="md:hidden">
    <v-bottom-navigation
      :grow="true"
      @update:modelValue="bottomNavItemClicked"
      bg-color="#050505"
      color="#aaa"
      density="comfortable"
      mandatory
      rounded
      v-model="defaultNavBarItem"
    >
      ...
      <v-btn
        v-if="status === 'unauthenticated' || status === 'loading'"
        value="profile"
        @click="signIn('google')"
      >
  ...
  >
    <div class="md:flex md:justify-center relative h-full">
      <div
        class="absolute max-md:w-full md:px-10 md:top-36 md:flex md:flex-col md:items-stretch md:w-1/3"
      >
        <v-autocomplete
          auto-select-first
          label="Search"
                  ...
                  <div class="max-md:w-1/3 md:w-1/4 h-full">
                    <SeoImg
                      :src="`https://image.tmdb.org/t/p/${
                        configuration.images.poster_sizes.w185
...
const doSearch = async (query: string) => {
  ...
  let searchResponse = await $fetch(
    `/api/search?query=${encodeURIComponent(query)}&type=${selectedType.value}`
  );
