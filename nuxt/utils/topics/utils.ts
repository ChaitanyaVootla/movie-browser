...
export const getGenreMeta = (genreString: string, media = "movie") => {
  const genre =
    media === "movie"
      ? Object.values(movieGenres).find(
          ({ name }) => sanitiseTopic(name) === sanitiseTopic(genreString)
        )
      : Object.values(seriesGenres).find(
          ({ name }) => sanitiseTopic(name) === sanitiseTopic(genreString)
        );
  return {
