...
const countryTopics = getCodes()
  .map((countryCode) => [getCountryMeta(countryCode), getCountryMeta(countryCode, "tv")])
  .flat();

const languageTopics = LANGAUAGES.map(({ iso_639_1 }) => [
  getLanguageMeta(iso_639_1),
  getLanguageMeta(iso_639_1, "tv"),
]).flat();
