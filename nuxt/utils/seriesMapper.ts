export const mapWatchListSeries = (series: any[]) => {
  series = series.map((item: any) => {
    ...
    const dateToSort = new Date(
      next_episode_to_air?.air_date || last_episode_to_air?.air_date || 0
    ).getTime();
  ...
  return {
    currentRunningSeries: currentRunningSeries.sort(
      (a: any, b: any) => a.dateToSort - b.dateToSort
    ),
    returingSeries,
    completedSeries,
