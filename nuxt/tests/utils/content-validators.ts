...
export class ContentValidator {
  ...
  async validateSSRStructure(pageType: string): Promise<{
    ...
    const hasLoadingSpinners =
      $('.loading, [class*="loading"]:not([class*="v-"]):not([aria-label])').length > 0;
  ...
  private validateImages($: any): {
    ...
    if (totalImages > 0 && !hasGoodSuccessRate) {
      issues.push(
        `Only ${properImages}/${totalImages} images (${Math.round(imageSuccessRate * 100)}%) have proper attributes`
      );
      isValid = false;
    }
  ...
  async validateHomepageContent(): Promise<ContentValidationResult> {
    ...
    const navLinks = $(
      '.navbar a, .v-bottom-navigation a, a[href*="/movie/"], a[href*="/series/"], a[href*="/browse"], a[href*="/topics"]'
    ).length;
    ...
    const hasRealContent =
      contentLength > 500 && hasRealMovieTitles && !hasSkeletonElements && !hasLoadingText;
  ...
  async validateMoviePageContent(movieId: string): Promise<ContentValidationResult> {
    ...
    const hasOverview =
      $("div, p, section").filter((_, el) => {
        const text = $(el).text();
        return (text.length > 100 && text.toLowerCase().includes("overview")) || text.length > 200; // Long descriptive text
      }).length > 0;
    ...
    const navLinks = $(
      'nav a, header a, .navbar a, .v-bottom-navigation a, a[href*="/movie/"], a[href*="/series/"], a[href*="/person/"], a[href*="/topics"], a[href*="/browse"]'
    ).length;
    ...
    return {
      ...
      hasUserActions:
        ratingElements > 0 ||
        watchlistButtons > 0 ||
        $('button, [role="button"], .v-btn').length > 0,
      contentQuality: {
  ...
  async validateSeriesPageContent(seriesId: string): Promise<ContentValidationResult> {
    ...
    const hasEpisodes =
      $('[class*="episode"], [data-episode]').length > 0 ||
      bodyText.toLowerCase().includes("episode");
    const hasSeason =
      bodyText.toLowerCase().includes("season") || $('select, [class*="season"]').length > 0;
  ...
  async validateSSRQuality(): Promise<{
    ...
    const hasJavaScriptDependencies =
      bodyText.includes("JavaScript is required") ||
      bodyText.includes("Enable JavaScript") ||
      (content.includes("noscript") && $("noscript").text().includes("JavaScript"));
   ...
   */
  async runFullContentValidation(
    pageType: "homepage" | "movie" | "series" | "person",
    id?: string
  ) {
    ...
    expect(
      ssrStructure.hasVisibleContent,
      "Page should have visible content without JavaScript"
    ).toBe(true);
    expect(ssrStructure.hasWorkingLinks, "Page should have working internal links").toBe(true);
    expect(ssrStructure.hasProperImages, "Images should have proper src and alt attributes").toBe(
      true
    );
    expect(ssrStructure.contentBlocked, "Content should not be blocked by loading states").toBe(
      false
    );
    ...
    console.log(
      `📊 Content stats: ${pageValidation.contentQuality.textLength} chars, ${pageValidation.contentQuality.linkCount} links`
    );
