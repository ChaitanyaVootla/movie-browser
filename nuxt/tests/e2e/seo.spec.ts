...
test.describe("SEO Validation - Critical Pages", () => {
  ...
  test("Homepage SEO validation", async ({ page }) => {
    ...
    expect(contentValidation.hasSeriesContent, "Homepage should load with series content").toBe(
      true
    );
    expect(
      contentValidation.contentQuality.hasRealContent,
      "Homepage should have real content, not loading states"
    ).toBe(true);
    expect(
      contentValidation.contentQuality.linkCount,
    ...
    console.log(
      `✅ Homepage loaded with ${contentValidation.contentQuality.linkCount} navigation links`
    );
  });
