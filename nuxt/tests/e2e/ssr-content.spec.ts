...
test.describe("SSR Content Validation - Critical Content Without JavaScript", () => {
  ...
  test("Homepage loads with complete movie/series content (no JS)", async ({ page }) => {
    ...
    expect(
      result.pageValidation.contentQuality.linkCount,
      "Homepage should have many navigation links"
    ).toBeGreaterThan(15);
    ...
    expect(
      result.pageValidation.contentQuality.textLength,
      "Homepage should have substantial content"
    ).toBeGreaterThan(1000);
    expect(
      result.pageValidation.contentQuality.hasLoadingStates,
      "Homepage should not show loading states in SSR"
    ).toBe(false);
    ...
    expect(
      result.pageValidation.contentQuality.imageCount,
      "Homepage should have multiple images for movie/series posters"
    ).toBeGreaterThan(10);
    expect(
      result.ssrStructure.hasProperImages,
      "Homepage images should be properly rendered in SSR"
    ).toBe(true);

    console.log(
      `✅ Homepage loaded with ${result.pageValidation.contentQuality.linkCount} links, ${result.pageValidation.contentQuality.imageCount} images, and ${result.pageValidation.contentQuality.textLength} characters`
    );
  });
  ...
  test("Movie page loads with complete content (no JS)", async ({ page }) => {
    ...
    expect(result.pageValidation.hasMovieContent, "Movie page should have movie content").toBe(
      true
    );
    expect(result.pageValidation.hasPersonContent, "Movie page should have cast links").toBe(true);
    expect(
      result.pageValidation.hasUserActions,
      "Movie page should have interactive elements"
    ).toBe(true);

    // Content quality for movie page
    expect(
      result.pageValidation.contentQuality.textLength,
      "Movie page should have detailed content"
    ).toBeGreaterThan(1500);
    expect(
      result.pageValidation.contentQuality.linkCount,
      "Movie page should have navigation and cast links"
    ).toBeGreaterThan(5);

    // Movie page specific image validation
    expect(
      result.pageValidation.contentQuality.imageCount,
      "Movie page should have poster, backdrop, and cast images"
    ).toBeGreaterThan(5);
    expect(
      result.ssrStructure.hasProperImages,
      "Movie page images should render properly in SSR"
    ).toBe(true);
  ...
  test("Series page loads with episodes/seasons (no JS)", async ({ page }) => {
    ...
    expect(result.pageValidation.hasSeriesContent, "Series page should have series content").toBe(
      true
    );
    ...
    expect(
      result.pageValidation.contentQuality.imageCount,
      "Series page should have poster, backdrop, and cast images"
    ).toBeGreaterThan(5);
    expect(
      result.ssrStructure.hasProperImages,
      "Series page images should render properly in SSR"
    ).toBe(true);

    // Check for series-specific elements
    const content = await page.content();
    const hasSeasonInfo =
      content.toLowerCase().includes("season") || content.toLowerCase().includes("episode");
    expect(hasSeasonInfo, "Series page should mention seasons or episodes").toBe(true);

    console.log(
      `✅ Series page loaded with season/episode information and ${result.pageValidation.contentQuality.imageCount} images`
    );
  });
  ...
  test("Person page loads with filmography (no JS)", async ({ page }) => {
    ...
    expect(result.pageValidation.hasPersonContent, "Person page should have person info").toBe(
      true
    );
    ...
    expect(
      result.pageValidation.contentQuality.imageCount,
      "Person page should have profile and filmography images"
    ).toBeGreaterThan(3);
    expect(
      result.ssrStructure.hasProperImages,
      "Person page images should render properly in SSR"
    ).toBe(true);

    // Person page should have substantial biography content
    expect(
      result.pageValidation.contentQuality.textLength,
      "Person page should have biography content"
    ).toBeGreaterThan(800);

    console.log(
      `✅ Person page loaded with filmography, biography, and ${result.pageValidation.contentQuality.imageCount} images`
    );
  });
  ...
  test("All critical pages have working navigation", async ({ page }) => {
    ...
    for (const { path, name } of pages) {
      ...
      const navCount = await page
        .locator(
          'a[href*="/movie/"], a[href*="/series/"], a[href*="/browse"], a[href*="/topics"], .navbar a, .v-bottom-navigation a'
        )
        .count();
...
test.describe("Content Quality Without JavaScript", () => {
  test("No loading states should be visible in SSR", async ({ page }) => {
    ...
    for (const path of pages) {
      ...
      const hasSkeletons =
        bodyText?.toLowerCase().includes("skeleton") || content.includes("v-skeleton-loader");
  ...
  test("Images have proper alt text for accessibility", async ({ page }) => {
    ...
    for (const img of images.slice(0, 10)) {
      ...
      if (
        ...
        !src.includes("login_small.svg")
      ) {
        // Skip system icons that don't need alt text
...
test.describe("SeoImg Component & Image Loading Tests", () => {
  ...
  test("SeoImg components render native img tags in SSR", async ({ page }) => {
    ...
    expect(
      nativeImages.length,
      "Should have native img tags from SeoImg components"
    ).toBeGreaterThan(5);
    ...
    expect(imagesWithSrc, "Most images should have proper src attributes").toBeGreaterThan(
      nativeImages.length * 0.7
    );
    expect(imagesWithAlt, "Most images should have descriptive alt text").toBeGreaterThan(
      nativeImages.length * 0.8
    );
    expect(imagesWithLoading, "Images should have proper loading attributes").toBeGreaterThan(
      nativeImages.length * 0.5
    );

    console.log(
      `✅ Found ${nativeImages.length} native img tags: ${imagesWithSrc} with src, ${imagesWithAlt} with alt, ${imagesWithLoading} with loading attrs`
    );
  });
  ...
  test("Image fallback system works (CDN → TMDB)", async ({ page }) => {
    ...
    console.log(
      `✅ Image sources: ${cdnImages} CDN, ${tmdbImages} TMDB out of ${totalImages} total`
    );
  });
  ...
  test("Image alt text is SEO-optimized", async ({ page }) => {
      ...
      if (src && !src.includes("skeleton")) {
        ...
        if (alt) {
          ...
          const hasContext =
            ...
            alt.includes("TV");
          const notGeneric =
            !alt.toLowerCase().includes("image") || !alt.toLowerCase().includes("picture");
    ...
    expect(
      altTextQuality,
      `Image alt text quality should be high. Bad examples: ${badAltExamples.slice(0, 3).join(", ")}`
    ).toBeGreaterThan(0.7);

    console.log(
      `✅ Alt text quality: ${goodAltText}/${totalImages} (${Math.round(altTextQuality * 100)}%) have descriptive alt text`
    );
  });
  ...
  test("Lazy loading attributes are properly set", async ({ page }) => {
    ...
    console.log(
      `✅ Loading attrs: ${lazyImages} lazy, ${eagerImages} eager out of ${totalImages} total`
    );
  });
