...
export default defineEventHandler(async (event) => {
  ...
  ratings.forEach((rating) =>
    rating.itemType === "movie" ? movieIds.push(rating.itemId) : seriesIds.push(rating.itemId)
  );
  if (movieIds.length) {
  ...
  return {
    likes: {
      movies: movies.filter((movie) =>
        ratings.find((rating) => rating.itemId === movie.id && rating.rating === 1)
      ),
      series: series.filter((serie) =>
        ratings.find((rating) => rating.itemId === serie.id && rating.rating === 1)
      ),
    },
    dislikes: {
      movies: movies.filter((movie) =>
