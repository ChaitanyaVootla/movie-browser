...
export default defineEventHandler(async (event) => {
  ...
  const [moviesList, seriesList] = await Promise.all([
    MoviesWatchList.find({ userId: userData?.sub })
  ...
  const [movies, series] = await Promise.all([
    Movie.find({ id: { $in: movieListIds } }).select(
      "-_id id title poster_path backdrop_path genres vote_average googleData overview images.backdrops"
    ),
    Series.find({ id: { $in: seriesListIds } }).select(
      "-_id id name poster_path next_episode_to_air last_episode_to_air status number_of_seasons images.backdrops"
    ),
  ]);
