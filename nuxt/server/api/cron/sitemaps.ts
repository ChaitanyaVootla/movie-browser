...
export const updateSiteMaps = async () => {
  ...
  updateMovieSitemaps()
    .then(() => updateSeriesSitemaps())
    .then(() => updatePersonSitemaps())
...
export const updateMovieSitemaps = async () => {
  ...
  while (true) {
    ...
    const URLS = voteCountMovies.map(
      (movie) => `${BASE_URL}/movie/${movie.id}/${getUrlSlug(movie.title as string)}`
    );
  ...
  while (true) {
    ...
    const dbMovies = await Movie.find({ id: { $in: chunk.map((movie) => movie.id) } })
      .select("-_id id title")
      .lean();
    const URLS = dbMovies.map(
      ({ id, title }) => `${BASE_URL}/movie/${id}/${getUrlSlug(title as string)}`
    );
    ...
    console.log(
      `Created sitemap for movies ${skipNonPriority + 1} to ${skipNonPriority + URLS.length}`
    );
...
export const updateSeriesSitemaps = async () => {
  ...
  while (true) {
    ...
    const URLS = voteCountSeries.map(
      ({ id, name }) => `${BASE_URL}/series/${id}/${getUrlSlug(name as string)}`
    );
  ...
  while (true) {
    ...
    const dbSeries = await Series.find({ id: { $in: chunk.map(({ id }) => id) } })
      .select("-_id id name")
      .lean();
    const URLS = dbSeries.map(
      ({ id, name }) => `${BASE_URL}/series/${id}/${getUrlSlug(name as string)}`
    );
    ...
    console.log(
      `Created sitemap for series ${skipNonPriority + 1} to ${skipNonPriority + URLS.length}`
    );
...
export const updatePersonSitemaps = async () => {
  ...
  for (let i = 0; i < priorityPersons.length; i += CHUNK_SIZE) {
    ...
    const URLS = chunk.map(
      ({ id, name }) => `${BASE_URL}/person/${id}/${getUrlSlug(name as string)}`
    );
  ...
  for (let i = 0; i < nonPriorityPersons.length; i += CHUNK_SIZE) {
    ...
    console.log(
      `Created sitemap for persons ${skipNonPriority + 1} to ${skipNonPriority + URLS.length}`
    );
