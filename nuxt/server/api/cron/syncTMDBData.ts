...
export const syncMovies = async () => {
  ...
  for (let i = 0; i < movieData.length; i += BATCH_SIZE) {
    ...
    console.log(
      `==================== Processing batch ${i / BATCH_SIZE + 1}/${Math.ceil(movieData.length / BATCH_SIZE)}`
    );

    const dbMovies: any = await Movie.find({
      id: { $in: batch.map((movie) => movie.id) },
    })
      .select("-_id id updatedAt shallowUpdatedAt")
      .lean();
    ...
    const moviesToUpdate = batch.filter((movie) => {
      ...
      return (
        dbMovie &&
        new Date(dbMovie.shallowUpdatedAt || dbMovie.updatedAt) <
    ...
    for (let j = 0; j < missingMovies.length; j += CHUNK_SIZE) {
      ...
      console.log(
        `------------- Updating chunk ${j / CHUNK_SIZE + 1}/${Math.ceil(missingMovies.length / CHUNK_SIZE)}`
      );
      await Promise.all(
        chunk.map((movie) => movieGetHandler(movie.id, true, true, true, true, false))
      );
      // await 5 secs
      await new Promise((resolve) => setTimeout(resolve, COOL_DOWN));
    }
...
export const syncSeries = async () => {
  ...
  for (let i = 0; i < seriesData.length; i += BATCH_SIZE) {
    ...
    console.log(
      `==================== Processing batch ${i / BATCH_SIZE + 1}/${Math.ceil(seriesData.length / BATCH_SIZE)}`
    );
    ...
    const seriesToUpdate = batch.filter((series) => {
      ...
      return (
        dbSeries &&
        new Date(dbSeries.shallowUpdatedAt || dbSeries.updatedAt) <
          new Date(Date.now() - 30 * ONE_DAY)
      );
    });
    ...
    for (let j = 0; j < missingSeries.length; j += CHUNK_SIZE) {
      ...
      console.log(
        `------------- Updating chunk ${j / CHUNK_SIZE + 1}/${Math.ceil(missingSeries.length / CHUNK_SIZE)}`
      );
      await Promise.all(
        chunk.map((series) => seriesGetHandler(series.id, true, true, true, true, false))
      );
      // await 5 secs
      await new Promise((resolve) => setTimeout(resolve, COOL_DOWN));
    }
