...
export default defineEventHandler(async (event) => {
  const allSeriesIds = (await SeriesList.find({}).select("seriesId -_id"))
    .map((doc) => doc.toJSON())
...
const updateSeriesById = async (seriesId: number) => {
  ...
  const series: any = await $fetch(
    `${TMDB.BASE_URL}/tv/${seriesId}?api_key=${process.env.TMDB_API_KEY}${SERIES_QUERY_PARAMS}`,
    {
      retry: 5,
    }
  ).catch(() => {
