...
export default defineEventHandler(async (event) => {
  ...
  const series = await seriesGetHandler(
    seriesId as string,
    checkUpdate,
    isForce,
    false,
  ...
  const latestSeasonNumber =
    series.next_episode_to_air?.season_number || series.last_episode_to_air?.season_number;
  let selectedSeason = {};
...
export const seriesGetHandler = async (
  seriesId: string,
  checkUpdate: boolean,
  isForce: boolean,
  ...
  if (country && /^[a-zA-Z]{2}$/.test(country)) {
    ...
    let query = Series.findOne({ id: seriesId }).select(
      "-_id -__v -images.posters -production_companies -production_countries -spoken_languages -similar"
    );
  ...
  if (isForce || !series.name) {
    ...
    try {
      const [details, watchProviders]: [any, any] = await Promise.all([
        $fetch(
          `${TMDB.BASE_URL}/tv/${seriesId}?api_key=${process.env.TMDB_API_KEY}${queryParams}`,
          {
            retry: 5,
          }
        ),
        $fetch(
          `${TMDB.BASE_URL}/tv/${seriesId}/watch/providers?api_key=${process.env.TMDB_API_KEY}`,
          {
            retry: 5,
          }
        ),
  ...
  const watchOptions = event
    ? getWatchOptions(event, series.googleData, series.watchProviders)
    : [];
...
const getCachedSeasonData = async (seriesId: string, seasonNumber: number): Promise<any> => {
  ...
  try {
    ...
    console.log(
      `🔄 Cache miss for season ${seasonNumber} of series ${seriesId}, fetching from TMDB...`
    );

    // Fetch from TMDB API
    const seasonData: any = await $fetch(
      `${TMDB.BASE_URL}/tv/${seriesId}/season/${seasonNumber}?api_key=${process.env.TMDB_API_KEY}`,
      {
        retry: 5,
      }
    );
