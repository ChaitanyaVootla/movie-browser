...
export default defineEventHandler(async () => {
  try {
    ...
    const allItemMoviefullInfo = await $fetch(
      `/api/movie/getMultiple?movieIds=${movieIdsToFetchFullData.join(",")}`
    );
    const allItemTvfullInfo = await $fetch(
      `/api/series/getMultiple?seriesIds=${allItemsTv
        .map((item: any) => item.id)
        .filter(Boolean)
        .join(",")}`
    );

    const mappedAllItems = allItems
      .map((item: any) => {
      ...
      .filter((item: any) => item.backdrop_path && item.vote_count > 10);

    return {
      allItems: mappedAllItems,
      movies: movies.map((movie: any) => {
        const fullInfo: any =
          allItemMoviefullInfo.find((fullMovie: any) => fullMovie.id === movie.id) || {};
        return {
          id: movie.id,
          title: movie.title,
      ...
      tv: tv.map((series: any) => {
        const fullInfo: any =
          allItemTvfullInfo.find((fullSeries: any) => fullSeries.id === series.id) || {};
        return {
          id: series.id,
          name: series.name,
