...
export default defineEventHandler(async (event) => {
  ...
  const userSpecificPatterns = ["/api/user/", "/api/auth/", "/api/admin/"];

  const isUserSpecific = userSpecificPatterns.some((pattern) => url.includes(pattern));
  ...
  if (url.includes("/api/")) {
    ...
    event.node.res.setHeader(
      ...
      "Content-Type, Authorization, Cache-Control, If-None-Match"
    );
