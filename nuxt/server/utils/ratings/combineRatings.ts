...
export const combineRatings = (
  ...
  if (externalData?.ratings) {
    ...
    if (
      externalData.ratings.rottenTomatoes?.critic &&
      externalData.ratings.rottenTomatoes.critic.score
    ) {
    ...
    if (
      externalData.ratings.rottenTomatoes?.audience &&
      externalData.ratings.rottenTomatoes.audience.score
    ) {
      const rtAudience = externalData.ratings.rottenTomatoes.audience;
      const normalizedRating = Math.round(
        normalizeRating(rtAudience.score, "Rotten Tomatoes (Audience)")
      );
      combinedRatings.push({
        rating: normalizedRating.toString(),
  ...
  if (googleData?.ratings) {
    for (const rating of googleData.ratings) {
      ...
      if (
        (sourceName.includes("tmdb") || sourceName.includes("the movie database")) &&
        addedSources.has("tmdb")
      )
        continue;
