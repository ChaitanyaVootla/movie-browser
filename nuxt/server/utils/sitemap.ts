...
export const updateSiteMapIndex = async () => {
  ...
  const seriesFileUrls = fs
    .readdirSync(seriesSitemapsDir)
    .map((file) => `${BASE_URL}/series/${file}`);
  const personFileUrls = fs
    .readdirSync(personSitemapsDir)
    .map((file) => `${BASE_URL}/person/${file}`);
