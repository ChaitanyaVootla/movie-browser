  ...
  <div :key="`${seriesUpdateKey}`">
    ...
    <div v-else-if="series?.id">
      ...
      <div>
        <div class="px-3 md:mx-12 mt-3">
          <div
            class="identify max-md:justify-center flex gap-2 md:gap-6 mb-0 md:mb-5 overflow-x-auto"
          >
          ...
          <div
            class="flex w-full items-start gap-4 flex-wrap max-md:justify-center md:justify-start max-md:mt-3 md:mt-5"
          >
            <div>
              <v-select
        ...
        <div class="px-3 md:px-0 max-md:-mt-2">
          <Scroller :items="series.selectedSeason?.episodes || []" title="" :pending="pending">
            <template v-slot:default="{ item }">
              <div
                class="flex flex-col gap-3 mt-2 cursor-pointer wide-card"
                @click="showEpisode(item)"
              >
                <div
                  class="text-neutral-400 overflow-ellipsis whitespace-nowrap overflow-hidden pr-4 text-xs md:text-sm -mb-1 flex items-center justify-between mr-1"
                >
                  <div class="flex gap-2">
                    ...
                    <div v-if="item?.air_date">
                      -
                      <NuxtTime
                        :datetime="new Date(item?.air_date)"
                        year="numeric"
                        month="long"
                        day="numeric"
                        class="ml-1"
                ...
                <div
                  class="text-neutral-200 overflow-ellipsis whitespace-nowrap overflow-hidden pr-4 text-xs md:text-base"
                >
                  {{ item?.name }}
                </div>
              </div>
        ...
        <div class="px-3 md:mx-12 overview max-md:mt-5 md:mt-10">
          <v-card class="px-4 py-4" color="#151515">
            ...
            <div class="flex items-center gap-5 flex-wrap mt-1">
              <h2>Overview</h2>
              <div
                v-if="series.created_by?.length"
                class="flex items-center gap-3 text-sm md:text-base"
              >
                <div class="text-neutral-300">Created by</div>
                <NuxtLink
                  :to="`/person/${person?.id}/${getUrlSlug(person.name)}`"
                  v-for="person in series.created_by"
                >
            ...
            <div class="flex items-center mt-3 gap-2">
              <NuxtLink
                v-if="series.origin_country?.length"
              ...
              <NuxtLink v-if="language" :to="languageTopicLink || '/'">
                ...
                >
                  <span
                    class="material-symbols-outlined !text-[22px] md:!text-xl text mr-1"
              ...
              <NuxtLink
                :key="`${isMounted}`"
                v-if="series.external_ids.imdb_id"
                :to="`https://www.imdb.com/title/${series.external_ids.imdb_id}/parentalguide`"
                target="blank"
                noreferrer
                noopener
                class="block"
            ...
            <div :key="`${isMounted}`" class="flex flex-wrap gap-2 md:gap-3 mt-2 md:mt-5">
              ...
              <v-chip
                v-if="series?.keywords?.results?.length > 5"
                @click="isKeywordsExpanded = !isKeywordsExpanded"
                class="rounded-pill cursor-pointer"
                :color="'#ddd'"
                :size="$vuetify.display.mdAndUp ? 'small' : 'x-small'"
                variant="text"
        ...
        <div
          v-if="series?.youtubeVideos?.length"
          class="px-3 md:px-20 max-md:mt-3 md:mt-10 md:hidden"
        >
          <Scroller
            :items="series?.youtubeVideos"
        ...
        <div
          v-if="series?.youtubeVideos?.length"
          class="px-3 md:px-20 max-md:mt-3 md:mt-5 max-md:hidden"
        >
        ...
        <div
          v-if="series.recommendations?.results?.length"
          class="px-3 md:px-0 max-md:mt-3 md:mt-10"
        >
          <Scroller
    ...
    >
      ...
      <p class="text-neutral-400 mb-2">
        {{ error.message || "An error occurred while loading the series." }}
      </p>
      <v-btn @click="refresh()" class="mt-4" variant="outlined" color="primary"> Try Again </v-btn>
    </div>
    <div v-else class="flex flex-col items-center justify-center min-h-[50vh] text-center">
      ...
      <v-btn @click="$router.push('/')" class="mt-4" variant="outlined"> Go Home </v-btn>
    </div>
    <Login ref="loginRef" />
    <v-dialog
    ...
    >
      <v-card class="episode-modal-card">
        ...
        <v-card-actions class="justify-end pa-2">
          <v-btn icon variant="text" @click="closeEpisodeDialog" class="episode-close-btn">
    ...
    <v-snackbar v-model="snackbar" :timeout="10000" color="black" timer="white">
      ...
      <template v-slot:actions>
        <v-btn color="white" size="small" variant="text" @click="snackbar = false"> Close </v-btn>
      </template>
    </v-snackbar>
  </div>
</template>
...
let language = computed(() => {
  if (series.value?.original_language !== "en") {
    return LANGAUAGES.find(({ iso_639_1 }) => iso_639_1 === series.value?.original_language)
      ?.english_name;
  }
  return null;
});
...
const mapSeries = (series: any) => {
  ...
  if (series.credits?.crew && series.created_by) {
    series.credits.crew.unshift(
      ...series.created_by.map((created_by: any) => ({
        ...created_by,
        job: "Created by",
      }))
    );
  ...
  return {
    ...series,
    youtubeVideos:
      (series.videos?.results?.filter((result: any) => result.site === "YouTube") || [])?.sort(
        (a: any, b: any) => {
...
const {
  ...
  error,
  refresh: refreshData,
} = await useFetch(`/api/series/${route.params.seriesId}`, {
...
let { data: watchlist }: any = await useFetch(
  `/api/user/series/${route.params.seriesId}/watchList`,
  {
...
const seasonSelected = async (season: any) => {
  const seasonData = await $fetch(
    `/api/series/${series.value.id}/season/${season?.season_number}`
  ).catch((err) => {
    console.log(err);
    return {};
  });
