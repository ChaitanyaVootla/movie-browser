    ...
    <div class="w-64 bg-neutral-950 flex flex-col justify-between px-4 pt-4 text-lg">
      <div>
        <div
          ...
          class="flex items-center gap-4 py-3 hover:bg-neutral-900 pl-4 cursor-pointer rounded-lg"
      ...
      <div>
        <div
          ...
          class="flex items-center gap-4 py-3 hover:bg-neutral-800 pl-4 cursor-pointer"
          @click="sidebarItemClicked(item)"
        >
          <span class="material-symbols-outlined !text-[22px] md:!text-3xl">
            {{ item.icon }}
    ...
    <div class="w-full px-10 pt-4">
      <div v-if="currentSidebarItem === 'likes'">
        ...
        <Grid
          v-if="isMovies"
          :items="likes?.movies || []"
          :title="`${likes?.movies?.length} movies`"
        />
      ...
      <div v-if="currentSidebarItem === 'watched'">
        <Grid
          :items="watchedMovies || []"
          :title="`${watchedMovies?.length} movies`"
          :pending="watchedFetchPending"
        />
      </div>
    </div>
  </div>
...
const { data: likes, refresh: refreshLikes } = await useLazyAsyncData(
  "userLikes",
  () => {
