  ...
  <div class="px-3 md:px-0">
    <div class="top-action flex justify-center mt-4">
      <v-btn-toggle
        ...
        mandatory
        density="compact"
        @update:model-value="viewUpdated"
      >
        <v-btn size="default"> Series </v-btn>
        <v-btn size="default"> Movies </v-btn>
      </v-btn-toggle>
    </div>
    <div>
      <div v-if="selectedType === 0" class="flex flex-col">
        ...
        <div v-if="watchListData?.series?.totalCount">
          <Scroller
            v-if="watchListData?.series?.currentRunningSeries?.length"
            :items="watchListData?.series?.currentRunningSeries"
            title="Running now"
            :pending="pending"
          />
          <Scroller
            v-if="watchListData?.series?.returingSeries?.length"
            :items="watchListData?.series?.returingSeries"
            title="Returning"
            :pending="pending"
            class="mt-4"
          />
          <Scroller
            v-if="watchListData?.series?.completedSeries?.length"
            :items="watchListData?.series?.completedSeries"
            title="Completed"
            :pending="pending"
            class="mt-4"
          />
        ...
        <div v-else>
          <div
            class="flex flex-col gap-5 justify-center text-2xl text-neutral-400 items-center mt-20"
          >
      ...
      <div v-else-if="selectedType === 1" class="max-md:px-3 md:px-14">
        <Grid
          v-if="watchListData?.movies?.length"
          :items="watchListData?.movies"
