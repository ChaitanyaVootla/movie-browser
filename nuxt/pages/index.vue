<template>
  ...
  <div v-if="trending?.allItems?.length || pending" class="-mb-5 md:-mb-5">
    <v-carousel
      ...
      delimiterIcon="mdi-circle-medium"
      class="carousel"
      :key="trending?.allItems?.length"
    >
      <template v-slot:prev="{ props }">
        <div
          class="w-12 h-full cursor-pointer flex justify-center items-center group"
          @click="props.onClick"
        >
  ...
  <div class="mt-5 md:mt-0 p-3 pb-16">
    <Scroller
      v-if="status === 'authenticated' && continueWatching?.length"
      :items="continueWatching"
      :pending="pending"
      title="Continue Watching"
      class="mb-4 md:pb-5"
      title-icon="mdi-play-circle-outline"
    >
    ...
    />
    <Scroller
      v-if="status === 'authenticated' && watchList?.ongoingSeries?.length"
      :items="watchList.ongoingSeries"
      :pending="false"
    ...
    />
    <Scroller
      v-if="status === 'authenticated' && watchList?.movies?.length"
      :items="watchList.movies"
      :pending="false"
      title="Movies in your watch list"
      class="mb-1 md:pb-5"
      title-icon="mdi-movie-open-outline"
    />
    <Scroller
      :items="trending?.streamingNow"
      :pending="pending"
      title="Streaming Now"
      class="mb-1 md:pb-5"
      title-icon="mdi-movie-open-outline"
    ...
    <Scroller
      v-if="status === 'authenticated' && recents?.length"
      :items="recents"
      :pending="pending"
      title="Recent visits"
      class="mb-1 md:pb-5"
...
const { pending, data: trending }: any = await useLazyAsyncData(
  "trending",
  () =>
...
const { data: watchList, refresh: refreshWatchList } = await useLazyAsyncData(
  "homeWatchList",
  () => {
...
const { data: continueWatching, refresh: refreshContinueWatching }: any = await useLazyAsyncData(
  "continueWatching",
  () => {
    if (status.value === "authenticated") {
