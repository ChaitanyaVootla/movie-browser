<template>
  <div>
    <div v-if="pending" class="pending w-full h-full">
      <v-skeleton-loader class="min-w-full h-full" max-width="300" type="image"></v-skeleton-loader>
    </div>
    <div v-else-if="person?.id">
        ...
        <div class="flex flex-col md:flex-row gap-5 md:gap-10">
          <div class="w-full md:w-1/12 md:min-w-32 flex justify-center md:block">
            <div class="w-48 md:w-full">
              <SeoImg
                :src="`https://image.tmdb.org/t/p/${configuration.images.profile_sizes.h632}${person.profile_path}`"
                :alt="person.name"
                class="rounded-lg"
                cover
              >
          ...
          <div class="w-full md:w-11/12">
            <div
              class="max-md:text-xl md:text-4xl font-bold flex flex-col md:flex-row md:items-baseline gap-2 md:gap-5 text-center md:text-left"
            >
              <h1>{{ person.name }}</h1>
            ...
            <div
              class="max-md:text-sm md:text-2xl text-neutral-400 flex flex-col md:flex-row md:align-baseline gap-1 md:gap-3 text-center md:text-left items-center md:items-baseline"
            >
            ...
            <div
              class="max-md:text-sm md:text-base text-neutral-400 mt-5 line-clamp-6 text-left max-md:text-center"
            >
              {{ person.biography }}
            </div>
          </div>
      ...
      <div class="max-md:px-3 md:px-14 max-md:mt-3 md:mt-10">
        <v-btn-toggle v-model="selectedMediaType" mandatory density="compact">
          ...
          <v-btn value="series"> Series </v-btn>
        </v-btn-toggle>
        <v-btn-toggle
          v-model="selectedCreditType"
          mandatory
          density="compact"
          class="max-md:ml-0 md:ml-10"
        >
          <v-btn value="cast"> Cast </v-btn>
          <v-btn value="crew"> Crew </v-btn>
        </v-btn-toggle>
    ...
    >
      ...
      <p class="text-neutral-400 mb-2">
        {{ error.message || "An error occurred while loading the person." }}
      </p>
...
const {
  ...
  error,
  refresh: refreshData,
}: any = await useFetch(`/api/person/${route.params.personId}`, {
