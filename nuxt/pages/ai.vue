    ...
    <div>
      <div class="flex gap-5 w-full">
        ...
        <v-btn
          ...
          v-bind:size="$vuetify.display.mobile ? 'default' : 'large'"
          @click="recommend"
          color="#ccc"
          >Search</v-btn
        >
      ...
      <div class="flex flex-wrap max-md:gap-2 md:gap-16 w-full">
        <div class="">
          ...
          <div>
            <v-btn-toggle
              class="mt-1"
              v-model="watchedQuery"
              @update:modelValue="recommend()"
              mandatory
              v-bind:density="$vuetify.display.mobile ? 'compact' : 'comfortable'"
            >
              <v-btn
                value="similar"
                text="Similar"
                v-bind:size="$vuetify.display.mobile ? 'x-small' : 'default'"
              />
              <v-btn
                value="neutral"
                text="Neutral"
...
const {
  ...
  execute: recommend,
  pending,
} = await useLazyAsyncData("results", () => {
  results.value = [];
